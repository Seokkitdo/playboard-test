<script lang="ts">
  ...
  let { selectedModule, selectedScreen, scrollToComponent, onComponentSelect }: Props = $props();
    ...
    switch (type) {
      case 'card':
        return [
          ...
          { name: 'variant', type: "'default' | 'outlined'", description: '카드 스타일 변형' }
        ];
      case 'button':
        return [
          { name: 'label', type: 'string', description: '버튼 텍스트' },
          { name: 'variant', type: "'primary' | 'secondary' | 'outlined'", description: '버튼 스타일' },
          { name: 'size', type: "'sm' | 'md' | 'lg'", description: '버튼 크기' },
      ...
      case 'table':
        return [
          ...
          { name: 'pagination', type: 'boolean', description: '페이지네이션 사용 여부' }
        ];
      default:
        return [];
    }
...
<main class="flex-1 h-full overflow-hidden bg-slate-950 flex flex-col">
  {#if selectedModule && selectedScreen}
    ...
    <div class="flex-1 flex overflow-hidden relative">
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4" bind:this={scrollContainer}>
        <div class="flex items-center justify-between mb-4">
          ...
          >
            <span class="text-sm">{isSidebarOpen ? '사이드바 닫기' : '사이드바 열기'}</span>
            <svg
          ...
          >
            <div class="p-4 border-b border-slate-700">
              <div class="flex items-center justify-between">
                <div>
                  <h3 class="text-base font-semibold text-slate-100 mb-1">{component.name}</h3>
                  <p class="text-sm text-slate-400">{component.description || ''}</p>
                </div>
                <span class="inline-block px-3 py-1 text-xs bg-slate-700 text-slate-300 rounded-full">
                  {component.type}
                </span>
              </div>
            </div>
            <div class="p-6 bg-slate-900/30">
              ...
              {:else}
                <div class="p-4 border border-slate-600 rounded-lg bg-slate-700 text-center">
                  <h3 class="font-semibold text-slate-100">{component.name}</h3>
                  <p class="text-xs text-slate-400 mt-2">타입: {component.type}</p>
                </div>
              {/if}
            </div>
          </div>
      ...
      >
        {#if isSidebarOpen}
          <div class="p-6 space-y-6 w-96">
            <!-- Props Section -->
            <div>
              <h2 class="text-lg font-semibold text-slate-200 mb-3">Props 정보</h2>
              {#if selectedComponent}
                <div class="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
                  <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                      <span class="text-slate-400">컴포넌트:</span>
                      <span class="text-slate-200">{selectedComponent.name}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-slate-400">타입:</span>
                      <span class="text-slate-200">{selectedComponent.type}</span>
                    </div>
                    {#if selectedComponent.description}
                      ...
                      <div>
                        ...
                        <p class="text-slate-300 text-xs">{selectedComponent.description}</p>
                      </div>
                    {/if}
                    <div class="border-t border-slate-700 my-3"></div>
                    ...
                    <div class="space-y-2">
                      {#each getComponentProps(selectedComponent.type) as prop}
                        <div class="pl-3 bg-slate-900/50 rounded p-2">
                          ...
                          <div class="text-xs text-slate-500 mt-1">{prop.description}</div>
                        </div>
                      {/each}
                    </div>
                  </div>
            ...
            {#if selectedComponent}
              <!-- Prompt Section -->
              <div>
                <h2 class="text-lg font-semibold text-slate-200 mb-3">Prompt</h2>
                <div class="space-y-3">
                  <textarea
              ...
              <!-- History Section -->
              <div>
                <h2 class="text-lg font-semibold text-slate-200 mb-3">수정 이력</h2>
                <div class="space-y-2">
